...
module CosmosDB =
    ...
    let get<'T> conId partKey docId : 'T  =
        let pk = getPartKey partKey
        dbClient.GetContainer(dbName, conId.ToString()).ReadItemAsync<'T>(docId.ToString(), partitionKey = pk).Result.Resource

    /// Fetch a item using the id property. Return None if the item does not exist.
    let tryGet<'T> conId partKey docId : 'T option =
        let pk = getPartKey partKey
        try
            Some (get<'T> conId partKey docId)
        with
        | :? System.AggregateException as ex ->
            match ex.GetBaseException() with
            | :? CosmosException as ce ->
    ...
    let deleteAll conId partKey =
        ()
        // let sql = "SELECT Value c.id from c" //list of ids
